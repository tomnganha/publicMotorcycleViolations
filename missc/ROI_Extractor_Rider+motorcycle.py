...
img_folder = r"C:\Users\Dev Agarwal\Desktop\vehicle_detection\Evaluation\Helmet_Data\HELMET_DATASET\test_helmet_data"
txt_folder = r"C:\Users\Dev Agarwal\Desktop\vehicle_detection\Evaluation\Helmet_Data\big_bbox+HNH\big_bbox_txt"
roi_folder = r"C:\Users\Dev Agarwal\Desktop\vehicle_detection\Evaluation\Helmet_Data\big_bbox+HNH\big_bbox_roi"
...
for files in glob.glob(txt_folder+"/*.txt"):
    ...
    for i in range(len(patch_)):
        p = patch_.iloc[i]
        xmax = float(p['x'] + p['w']/2)
        xmin = float(p['x'] - p['w']/2)
        ymax = float(p['y'] + p['h']/2)
        ymin = float(p['y'] - p['h']/2)
